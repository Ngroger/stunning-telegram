...
async def get_type(message: Message, state: FSMContext):
    ...
    if qty < 0.001:
        ...
        # Логирование добавления данных в базу
        print(f"Added user {user_id} with account {account} and quantity {qty} to the database.")
...
async def check_wallet(message: Message):
    ...
    if result is not None:
        ...
        if account in ['UNIFIED', 'CONTRACT']:
            try:
                balance_result = check_balance(account)
                if balance_result is not None:
                    totalwalletbalance, availablebalance = balance_result
                    await message.answer(f'Ваш баланс: {totalwalletbalance} | USDT \U0001F4B3\n'
                                         f'Доступные средства: {availablebalance} \U0001F4B8')
                else:
                    await message.answer('Ошибка при получении баланса, попробуйте позже \U0001F4A9')
            except Exception as e:
                await message.answer(f'Произошла ошибка при получении баланса: {str(e)}')
        else:
...
async def main():
    ...
    cur.execute("INSERT INTO user_account (user_id, account, qty) VALUES (?, ?, ?)", (649430328, 'UNIFIED', 0.001))
